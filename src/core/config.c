...
void config_normalize(BomberConfig* cfg) {
    ...
    cfg->flame_duration = clamp_int(cfg->flame_duration, 1, 32);
    if (cfg->sudden_death_start < 0) cfg->sudden_death_start = 0;
    cfg->shrink_interval = clamp_int(cfg->shrink_interval, 1, 1000);
...
void config_defaults(BomberConfig* cfg) {
    ...
    cfg->sudden_death_start = 0; /* disabled by default; battle/training opt in */
