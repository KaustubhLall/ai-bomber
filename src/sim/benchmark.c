...
static void emit_row(FILE* f, int json, int* first, const char* suite, uint64_t seed,
    ...
    if(json){if(!*first)fprintf(f,",\n");*first=0;fprintf(f,"    {\"seed_suite\":\"%s\",\"seed\":%llu,\"agent\":\"%s\",\"opponent\":\"%s\",\"episodes\":%d,\"win_rate\":%.6f,\"death_rate\":%.6f,\"draw_timeout_rate\":%.6f,\"avg_reward\":%.6f,\"avg_length\":%.6f,\"crates_destroyed\":%d,\"enemies_killed\":%d,\"self_kills\":%d,\"bombs_placed\":%d,\"powerups_collected\":%d,\"steps_per_sec\":%.3f}",suite,(unsigned long long)seed,agent_type_name(a),agent_type_name(b),m->episodes,m->wins/eps,m->deaths/eps,(m->draws+m->timeouts)/eps,m->total_reward/eps,m->total_steps/eps,m->crates_destroyed,m->enemies_killed,m->self_kills,m->bombs_placed,m->powerups_collected,seconds>0?m->total_steps/seconds:0.0);
    }else fprintf(f,"%s,%llu,%s,%s,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%d,%d,%.3f\n",suite,(unsigned long long)seed,agent_type_name(a),agent_type_name(b),m->episodes,m->wins/eps,m->deaths/eps,(m->draws+m->timeouts)/eps,m->total_reward/eps,m->total_steps/eps,m->crates_destroyed,m->enemies_killed,m->self_kills,m->bombs_placed,m->powerups_collected,seconds>0?m->total_steps/seconds:0.0);
}
...
int benchmark_matrix(const BomberConfig* cfg,int episodes,uint64_t seed,const char*suite,const char*path,int json){
    ...
    if(json)fprintf(f,"{\n  \"schema_version\": 1,\n  \"git_sha\": \"%s\",\n  \"run_config\": {\"episodes_per_matchup\":%d,\"seed\":%llu,\"seed_suite\":\"%s\",\"map\":{\"width\":%d,\"height\":%d,\"crate_density\":%d,\"bomb_timer\":%d}},\n  \"hardware\": \"native-c17\",\n  \"policy_config\": {\"alpha_beta_depth\":3,\"mcts_simulations\":48,\"mcts_rollout_depth\":3},\n  \"results\": [\n",AI_BOMBER_GIT_SHA,episodes,(unsigned long long)seed,suite,cfg->width,cfg->height,cfg->crate_density,cfg->bomb_timer);else fprintf(f,"seed_suite,seed,agent,opponent,episodes,win_rate,death_rate,draw_timeout_rate,avg_reward,avg_length,crates_destroyed,enemies_killed,self_kills,bombs_placed,powerups_collected,steps_per_sec\n");
