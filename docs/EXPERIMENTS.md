...
python tools/summarize_matrix.py results/holdout-matrix.json
```

Report head-to-head results across both role/spawn orientations. A policy that
only wins as agent 0 has not demonstrated a robust advantage. MCTS is an online
planner, not a persistent learner; improvement across runs requires a learned
policy/value checkpoint such as the AlphaZero-lite or PPO path.
